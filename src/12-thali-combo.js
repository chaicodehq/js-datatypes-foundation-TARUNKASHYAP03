...
export function createThaliDescription(thali) {
  ...
  const vegtext = thali.isVeg ? "Veg" : "Non-Veg";

  return `${name} (${vegtext}) - Items: ${items} - Rs.${price}`;
}
...
export function getThaliStats(thalis) {
  ...
  const sumPrice = thalis.reduce((sum, t) => sum + t.price, 0);
  const avgPrice = (sumPrice / totalThalis).toFixed(2);
